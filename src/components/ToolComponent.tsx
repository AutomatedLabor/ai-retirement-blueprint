...
function FreedomCalculator() {
  ...
  const monthlySavingsNeeded = Math.round((freedomNumber * (withdrawalRate / 100)) / 12);
  const yearsToGoal = Math.round(freedomNumber / (monthlySavingsNeeded * 12 * (1 + 0.07) ** 5));

  const handleExport = () => {
    const report = `FREEDOM NUMBER CALCULATOR - Results Report\nGenerated by AI Retirement Blueprint\n${"=".repeat(50)}\n\nYOUR FREEDOM NUMBER: $${freedomNumber.toLocaleString()}\n\nBased on annual expenses of $${expenses.toLocaleString()} and a ${withdrawalRate}% withdrawal rate, your Freedom Number is $${freedomNumber.toLocaleString()}.\n\nThis means you need a portfolio of $${freedomNumber.toLocaleString()} invested to sustain $${monthlyExpenses.toLocaleString()}/month in passive income indefinitely.\n\nKEY METRICS:\n- Annual Expenses: $${expenses.toLocaleString()}
- Monthly Expenses: $${monthlyExpenses.toLocaleString()}
- Safe Withdrawal Rate: ${withdrawalRate}%
- Required Portfolio: $${freedomNumber.toLocaleString()}
\nMONTHLY SAVINGS TARGET:\nTo reach $${freedomNumber.toLocaleString()} in 10 years at a 7% annual return, you need to save approximately $${Math.round(freedomNumber / 173).toLocaleString()}/month.\n\nWHAT THIS MEANS:\nOnce you reach $${freedomNumber.toLocaleString()} in invested assets, you can withdraw $${monthlyExpenses.toLocaleString()}/month (adjusted for inflation) and have a 95%+ probability of never running out of money over a 30-year retirement.\n\nNEXT STEPS:\n1. Review your current expenses and see where you can optimize\n2. Calculate how much you need to save each month to reach this number\n3. Set up automatic investments toward your freedom number\n4. Track your progress monthly using the Retirement Timeline tool\n\n${"=".repeat(50)}\nGenerated by AI Retirement Blueprint - ai-retirement-blueprint.vercel.app`;
    downloadReport("freedom-number-report.txt", report);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <DownloadButton onClick={handleExport} label="Download Results as Report" />
    </div>
  );
}
...
function RetirementTimeline() {
  ...
  const handleExport = () => {
    ...
    const report = `RETIREMENT TIMELINE - Results Report\nGenerated by AI Retirement Blueprint\n${"=".repeat(50)}\n\nYOUR RETIREMENT TIMELINE\n\nBased on current savings of $${savings.toLocaleString()} and monthly savings of $${monthlySavings.toLocaleString()}, your projected portfolio at age ${targetAge} is $${finalBalance.toLocaleString()}.\n\nKEY INPUTS:\n- Current Age: ${currentAge}\n- Target Retirement Age: ${targetAge}\n- Years of Growth: ${years}\n- Current Savings: $${savings.toLocaleString()}\n- Monthly Savings: $${monthlySavings.toLocaleString()}\n- Annual Return: ${annualReturn}%\n\nPROJECTED GROWTH:\nYear | Age | Portfolio Value\n${"-".repeat(35)}`;
    const yearLines = data.filter((_, i) => i % Math.max(1, Math.floor(years / 10)) === 0 || i === years).map(d => `${d.year.toString().padStart(4)} | ${String(d.age).padStart(3)} | $${d.balance.toLocaleString().padStart(12)}`).join("\n");
    const reportFull = report + "\n" + yearLines + `\n\nTOTAL CONTRIBUTED: $${(savings + monthlySavings * 12 * years).toLocaleString()}\nINVESTMENT GROWTH: $${(finalBalance - savings - monthlySavings * 12 * years).toLocaleString()}\n\nNEXT STEPS:\n1. Increase your monthly savings if possible to accelerate your timeline\n2. Review your investment allocation to ensure it matches your risk tolerance\n3. Consider side income streams to boost your savings rate\n4. Revisit this calculator quarterly to track your progress\n\n${"=".repeat(50)}\nGenerated by AI Retirement Blueprint - ai-retirement-blueprint.vercel.app`;
    downloadReport("retirement-timeline-report.txt", reportFull);
  };
...
function SkillsAssessmentQuiz() {
  ...
  const questions = [
    ...
    { q: "How skilled are you at editing and refining AI-generated content to match brand voice?", cat: 0 },
    // Coding (3 questions)
    ...
    { q: "How proficient are you at using AI to generate tests, documentation, and boilerplate code?", cat: 1 },
    ...
    { q: "How well can you integrate AI APIs (OpenAI, Claude) into custom workflows?", cat: 2 },
    ...
    { q: "How skilled are you at using AI for financial modeling and pricing analysis?", cat: 3 },
    // Design (3 questions)
    ...
    { q: "How well can you use AI to create presentation decks, infographics, and UI mockups?", cat: 4 },
  ];
  ...
  const getCatStrengths = (cat: string, score: number) => {
    if (score >= 4) return [`Strong capability in ${cat} — you can lead projects and mentor others`];
    if (score >= 3) return [`Solid foundation in ${cat} — focus on advanced techniques to reach expert level`];
    return [];
  };
  ...
  const nextStepsAll: Record<string, string[]> = {
    Content: [
      "Write 5 blog posts this week using AI for 80% of the first draft",
      "Create a content calendar using AI research for topic ideas",
      "Practice editing AI content to match 3 different brand voices",
    ],
    Coding: [
      "Use Cursor/Copilot exclusively for your next coding project",
      "Generate unit tests for an existing codebase using AI",
      "Refactor one module per week using AI suggestions",
    ],
    Automation: [
      "Identify 3 repetitive tasks and automate one this week",
      "Build a Zapier automation connecting your email, calendar, and task manager",
      "Explore one AI API and build a simple integration",
    ],
    Strategy: [
      "Create a SWOT analysis for your business using ChatGPT",
      "Use AI to research 3 competitors and identify gaps",
      "Build a pricing model using AI-powered financial analysis",
    ],
    Design: [
      "Generate 20 images in Midjourney and select the best 5 for refinement",
      "Create a complete brand kit (logo, colors, fonts) using AI tools",
      "Design a presentation deck for a real project using AI assistance",
    ],
  };
  ...
  const handleDownloadReport = () => {
    const strengths = categories.map((cat, i) => catScores[i] >= 4 ? cat : null).filter(Boolean);
    const weaknesses = categories.map((cat, i) => catScores[i] > 0 && catScores[i] < 3 ? cat : null).filter(Boolean);
    const notStarted = categories.map((cat, i) => catScores[i] === 0 ? cat : null).filter(Boolean);

    const catBreakdown = categories.map((cat, i) => {
      const level = getLevel(catScores[i]);
      const bar = "█".repeat(catScores[i]) + "░".repeat(5 - catScores[i]);
      return `  ${cat.padEnd(15)} ${bar} ${catScores[i]}/5  (${level})`;
    }).join("\n");

    const strengthLines = strengths.length > 0 ? strengths.map(s => `  ✦ ${s} — strong`).join("\n") : "  No category scored 4 or above yet.";
    const weaknessLines = weaknesses.length > 0 ? weaknesses.map(w => `  ⚠ ${w} — needs focused improvement`).join("\n") : "  No low-scoring categories.";
    const notStartedLines = notStarted.length > 0 ? notStarted.map(n => `  O ${n} - untapped opportunity`).join("\n") : "  All categories have been assessed.";

    const recLines = categories.map((cat, i) => {
      if (catScores[i] >= 4) { return "\n" + cat.toUpperCase() + " (" + catScores[i] + "/5):\n  * Already at advanced level. Focus on mentoring and leading.\n  * Next step: " + nextStepsAll[cat][0]; }
      const recs = recommendations[cat];
      const steps = nextStepsAll[cat];
      const numRecs = catScores[i] === 0 ? 4 : Math.max(2, 4 - catScores[i]);
      const recList = recs.slice(0, numRecs).map(r => "  - " + r).join("\n");
      const stepList = steps.slice(0, 2).map((s, si) => "  " + (si + 1) + ". " + s).join("\n");
      return "\n" + cat.toUpperCase() + " (" + catScores[i] + "/5 - " + getLevel(catScores[i]) + "):\n" + recList + "\n  IMMEDIATE NEXT STEPS:\n" + stepList;
    }).join("\n");

    const weakest = categories.filter((_, i) => catScores[i] > 0).sort((a, b) => catScores[categories.indexOf(a)] - catScores[categories.indexOf(b)]);
    const untouched = categories.filter((_, i) => catScores[i] === 0);
    const priorityPlan = [...untouched, ...weakest.slice(0, 2)].map((cat, idx) => "  " + (idx + 1) + ". " + cat + " - Start with: " + recommendations[cat][0]).join("\n");

    const sep = "=".repeat(55);
    const sep2 = "-".repeat(55);
    const report = `AI SKILLS ASSESSMENT - Detailed Report\nGenerated by AI Retirement Blueprint\n${sep}\n\nOVERALL SCORE: ${overallScore}/5 (${getLevel(overallScore)})\n\nCATEGORY BREAKDOWN:\n${sep2}\n${catBreakdown}\n\nSTRENGTHS (Score 4-5):\n${strengthLines}\n\nDEVELOPMENT AREAS (Score 1-2):\n${weaknessLines}\n\nNOT YET STARTED (Score 0):\n${notStartedLines}\n\nDETAILED RECOMMENDATIONS BY CATEGORY:\n${sep2}\n${recLines}\n\nPRIORITY ACTION PLAN:\n${sep2}\n${priorityPlan}\n\n${sep}\nGenerated by AI Retirement Blueprint - ai-retirement-blueprint.vercel.app`;
    downloadReport("ai-skills-assessment-report.txt", report);
  };
    ...
    return (
      <div className="space-y-6">
        <div className="bg-gradient-to-br from-primary-50 to-white rounded-xl p-8 border text-center">
          ...
          <div className="text-sm text-gray-500 mt-1">{getLevel(overallScore)}</div>
        </div>
        ...
        {/* Category Breakdown with Levels */}
        <div className="grid grid-cols-5 gap-2">
          {categories.map((cat, i) => (
            <div key={cat} className="bg-white rounded-lg p-3 border text-center">
              ...
              <div className="text-[10px] text-gray-400 mt-0.5">{getLevel(catScores[i])}</div>
            </div>
          ))}
        </div>
        {/* Strengths Summary */}
        {categories.some((_, i) => catScores[i] >= 4) && (
          <div className="bg-accent-50 dark:bg-accent-900/20 rounded-xl p-5 border border-accent-200">
            <h3 className="font-bold text-accent-800 mb-2">💪 Strengths</h3>
            <ul className="space-y-1">
              {categories.map((cat, i) => catScores[i] >= 4 ? (
                <li key={cat} className="flex gap-2 text-sm text-accent-900">
                  <span className="text-accent-500">✦</span>
                  <span><strong>{cat}</strong> — {getCatStrengths(cat, catScores[i])[0]}</span>
                </li>
              ) : null)}
            </ul>
          </div>
              ...
              <div key={cat} className="bg-white rounded-xl p-5 border">
                <div className="flex items-center gap-2 mb-3">
                  <span className={`text-sm font-semibold px-2 py-0.5 rounded-full ${catScores[i] === 0 ? 'bg-red-100 text-red-700' : catScores[i] <= 2 ? 'bg-gold-100 text-gold-700' : 'bg-primary-100 text-primary-700'}`}>
                    {cat}: {getLevel(catScores[i])} {catScores[i] > 0 && `(${catScores[i]}/5)`}
                  </span>
                </div>
                ...
                <div className="mt-3 pt-3 border-t border-gray-100">
                  <p className="text-xs font-medium text-gray-500">Immediate Next Step:</p>
                  <p className="text-sm text-primary-700 font-medium">→ {steps[0]}</p>
                </div>
              </div>
            );
          })}
        ...
        <div className="flex gap-3">
          <DownloadButton onClick={handleDownloadReport} label="Download Your Skills Report" />
        </div>
        <button onClick={() => { setStep(0); setAnswers(new Array(15).fill(0)); setShowResults(false); }} className="w-full py-3 bg-primary-600 text-white font-semibold rounded-xl hover:bg-primary-700 transition-colors">
          Retake Quiz
        </button>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="w-full bg-gray-200 rounded-full h-2">
        <div className="bg-primary-600 h-2 rounded-full transition-all duration-300" style={{ width: `${((step + 1) / questions.length) * 100}%` }}></div>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <DownloadButton onClick={() => {
        const recs = cats.filter(c => c.score < 70).map(c => `  - ${c.name} (${c.score}/100): ${c.tip}`).join("\n");
        const report = `FINANCIAL HEALTH SCORE - Results Report\nGenerated by AI Retirement Blueprint\n${"=".repeat(50)}\n\nYOUR FINANCIAL HEALTH SCORE: ${totalScore}/100\n\n${totalScore >= 80 ? "Excellent!" : totalScore >= 60 ? "Good foundation." : totalScore >= 40 ? "Fair — improvements needed." : "Needs immediate attention."}\n\nSCORE BREAKDOWN:\n${cats.map(c => `  ${c.name.padEnd(25)} ${String(c.score).padStart(3)}/100  (weight: ${c.weight})`).join("\n")}\n\nDETAILS:\n${cats.map(c => `  ${c.name}: ${c.detail} — ${c.tip}`).join("\n")}\n\nINPUT SUMMARY:\n  Monthly Income: $${monthlyIncome.toLocaleString()}\n  Monthly Expenses: $${monthlyExpenses.toLocaleString()}\n  Total Savings: $${Math.round(monthlySavings).toLocaleString()}\n  Total Debt: $${totalDebt.toLocaleString()}\n  Total Investments: $${totalInvestments.toLocaleString()}\n  Income Streams: ${incomeStreams}\n\nRECOMMENDATIONS:\n${recs || "  All areas are in good shape! Keep up the great work."}\n\n${"=".repeat(50)}\nGenerated by AI Retirement Blueprint - ai-retirement-blueprint.vercel.app`;
        downloadReport("financial-health-report.txt", report);
      }} label="Download Results as Report" />
    </div>
  );
}
